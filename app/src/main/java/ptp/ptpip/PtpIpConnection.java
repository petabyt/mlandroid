...
package fm.magiclantern.app.ptpip;

import fm.magiclantern.app.PtpDataType;
...
public class PtpIpConnection extends PtpTransport {
	...
	private final static int SESSION_ID =
		1; // only one session per connection for PtpIp - use this id
	...
	public static class PtpIpAddress extends PtpTransport.ResponderAddress {
		...
		public PtpIpAddress(InetAddress address) {
			this(address, PTP_PORT);
		}
		public PtpIpAddress(InetAddress address, int port) {
			mTcpAddress = new InetSocketAddress(address, port);
		}

		public String toString() {
			return mTcpAddress.toString();
		}
	}
	...
	public static class PtpIpHostId extends PtpTransport.HostId {
		...
		public PtpIpHostId(short[] guid, String friendlyName) {
			this(guid, friendlyName, PTP_VERSION_MAJOR, PTP_VERSION_MINOR);
		}
	...
	private class PtpIpPacketListener extends Thread {
		...
		private void putEvent(PtpEvent event) {
			if (mEventOutQueue == null)
				return;
			while (true) {
				try {
					mEventOutQueue.put(event);
					return;
				} catch (InterruptedException e) {
				}
			}
		...
		public void run() {
			...
			while (mStatus != ConnectionStatus.CLOSED) {
				...
				long remaining =
					PING_TIMEGAP - (System.currentTimeMillis() -
									Math.max(mCommandConnection.getLastActivityTimestamp(),
											 mEventConnection.getLastActivityTimestamp()));
				if (remaining <= 0) {
					// Sending ping when timing out; however, we're ignoring the pongs
					try {
						mEventConnection.sendPacket(new PtpIpPacket.ProbeRequest());
					} catch (IOException e) {
					}
					remaining = PING_TIMEGAP;
				}
				try {
					packet = mPacketInQueue.poll(remaining, TimeUnit.MILLISECONDS);
				} catch (InterruptedException e) {
					continue;
				}
				if (packet == null)
				...
				if (packet instanceof PtpIpPacket.LoadStatus) {
					if (((PtpIpPacket.LoadStatus)packet).mLoadedPacket instanceof
				...
				// if Ping --> send Pong
					...
					try {
						packet.getSourceConnection().sendPacket(new PtpIpPacket.ProbeResponse());
					} catch (IOException e) {
					}
				}
				...
				// if InitPacket --> check for correct state and send onwards
					...
					if (mStatus == ConnectionStatus.INITIALIZED)
						putBlocking(mInitPacketOutQueue, packet);
					else {
						LOG.severe(
							"PTPIP: Protocol violation (received InitPacket but not in Init state) - closing connection!");
						putFirstBlocking(
							mPacketInQueue,
							new PtpIpPacket.Error(new PtpIpExceptions.ProtocolViolation(
								"Wrong PacketType: Received InitPacket but not in Init state!")));
					}
				}
				...
				// if Event --> send a layer upwards (and cancel also onwards to be sure)
					...
					putEvent(new PtpEvent(new PtpDataType.EventCode(eventPacket.mEventCode),
										  new PtpDataType.UInt32(eventPacket.mTransactionId),
										  eventPacket.mParameters));
					if (eventPacket.mEventCode == PtpEvent.EVENTCODE_CancelTransaction)
						putBlocking(mTransactionPacketOutQueue, packet);
				}
				...
				// if TransactionPacket --> check for correct state and send onwards
					...
					if (mStatus == ConnectionStatus.CONNECTED)
						...
						LOG.severe(
							"PTPIP: Protocol violation (received TransactionPacket but not in Connected state) - closing connection!");
						putFirstBlocking(
							mPacketInQueue,
							new PtpIpPacket.Error(new PtpIpExceptions.ProtocolViolation(
								"Wrong PacketType: Received TransactionPacket but not in Connected state!")));
					}
				}
				...
				// if Other --> can't happen, there is nothing else --> Error
					...
					putFirstBlocking(mPacketInQueue,
									 new PtpIpPacket.Error(new PtpIpExceptions.MalformedPacket(
										 "Unknown internal packet type!")));
				}
			}
	...
	public PtpIpConnection(BlockingQueue<PtpEvent> eventOutQueue) {
		...
		if (eventOutQueue != null)
			mEventOutQueue = eventOutQueue;
	...
	public PtpDataType.DeviceInfoDataSet getDeviceInfo()
		...
		PtpOperation.Response response = mSingleSession.executeNullTransaction(
			PtpOperation.createRequest(PtpOperation.OPSCODE_GetDeviceInfo));
		response.validate();
		if (!response.isSuccess())
			throw new PtpIpExceptions.OperationFailed("GetDeviceInfo", response.getResponseCode());
		return (PtpDataType.DeviceInfoDataSet)response.getData();
	}

	@Override
	public PtpTransport.Session openSession() throws TransportDataError, TransportIOError,
		...
		response.validate();
		if (!response.isSuccess())
			throw new PtpIpExceptions.OperationFailed("OpenSession", response.getResponseCode());
		mSingleSession.setOpened(true);

		return mSingleSession;
	}

	protected void sendCommandChannelPacket(PtpIpPacket packet) throws IOException {
		mCommandConnection.sendPacket(packet);
	}
	protected void sendEventChannelPacket(PtpIpPacket.Event packet) throws IOException {
		mEventConnection.sendPacket(packet);
	}

	private PtpIpPacket.InitPacket connectChannel(TcpConnection tcpConnection,
												  InetSocketAddress address,
												  PtpIpPacket.InitPacket initPacket,
												  Class expectedAnswer)
		throws PtpIpExceptions.IOError, PtpIpExceptions.MalformedPacket,
			   PtpIpExceptions.ProtocolViolation, PtpIpExceptions.OperationFailed {
		try {
			tcpConnection.connect(address);
			tcpConnection.sendPacket(initPacket);
		} catch (IOException e) {
			close();
			throw new PtpIpExceptions.IOError("Could not connect channel!", e);
		}
		...
		while (packet == null) {
			try {
				packet = mInitPacketOutQueue.take();
			} catch (InterruptedException e) {
			}
		}

		if (packet instanceof PtpIpPacket.InitFail)
			throw new PtpIpExceptions.OperationFailed("InitRequest",
													  ((PtpIpPacket.InitFail)packet).mReason);
	...
	public void connect(PtpTransport.ResponderAddress address, PtpTransport.HostId hostId)
		...
		try {
			PtpIpPacket.InitPacket initAck =
				connectChannel(mCommandConnection, mAddress.mTcpAddress,
							   new PtpIpPacket.InitCommandRequest(mHostId),
							   PtpIpPacket.InitCommandAck.class);
			long mConnectionNumber = ((PtpIpPacket.InitCommandAck)initAck).mConnectionNumber;
			connectChannel(mEventConnection, mAddress.mTcpAddress,
						   new PtpIpPacket.InitEventRequest(mConnectionNumber),
						   PtpIpPacket.InitEventAck.class);
		} catch (PtpIpExceptions.IOError | PtpIpExceptions.MalformedPacket |
				 PtpIpExceptions.ProtocolViolation e) {
			close();
			throw e;
		}
	...
	public boolean isClosed() {
		return mStatus == ConnectionStatus.CLOSED;
	}

	public void close() {
		// stop listening
		mStatus = ConnectionStatus.CLOSED;
		if (mPtpIpPacketListener != null)
			mPtpIpPacketListener.interrupt();
