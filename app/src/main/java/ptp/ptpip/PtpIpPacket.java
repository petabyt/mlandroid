...
public abstract class PtpIpPacket {
	...
	private static final int DATAPHASE_UNKNOWN =
		0x00000003; // guessed value - not yet observed (but we're not using it anyway)
	...
	protected void setSourceConnection(TcpConnection tcpConnection) {
		mSourceConnection = tcpConnection;
	}
	...
	public String toString() {
		return "[Length: " + mLength + ", PacketType: " + String.format("0x%08x", mPacketType) +
			" (" +
	...
	public byte[] serializePacket() {
		...
		System.arraycopy(new DataBuffer(4).writeUInt32(packetData.length).getData(), 0, packetData,
						 0, 4);
	...
	interface ReadingListener {
		void onLoaded(PtpIpPacket packet, int loadedBytes);
	}
	public static PtpIpPacket readPacket(InputStream in)
		throws IOException, PtpIpExceptions.MalformedPacket {
		return readPacket(in, null);
	}
	public static PtpIpPacket readPacket(InputStream in, final ReadingListener listener)
		throws IOException, PtpIpExceptions.MalformedPacket {
		// -----------------------------------------------------------------------------------------
		...
		if (packetLength < 8)
			throw new PtpIpExceptions.MalformedPacket("PTP/IP PacketLength header < 8 bytes (" +
													  packetLength + ")");
		...
		case PKT_InitCommandRequest:
			packet = new InitCommandRequest();
			break;
		case PKT_InitCommandAck:
			packet = new InitCommandAck();
			break;
		case PKT_InitEventRequest:
			packet = new InitEventRequest();
			break;
		case PKT_InitEventAck:
			packet = new InitEventAck();
			break;
		case PKT_InitFail:
			packet = new InitFail();
			break;
		case PKT_OperationRequest:
			packet = new OperationRequest();
			break;
		case PKT_OperationResponse:
			packet = new OperationResponse();
			break;
		case PKT_Event:
			packet = new Event();
			break;
		case PKT_StartData:
			packet = new StartData();
			break;
		case PKT_Data:
			packet = new Data();
			break;
		...
		case PKT_ProbeRequest:
			packet = new ProbeRequest();
			break;
		case PKT_ProbeResponse:
			packet = new ProbeResponse();
			break;
		default:
			throw new PtpIpExceptions.MalformedPacket("Unknown packet type: " +
		...
		DataBuffer.LoadingListener loadListener =
			listener == null ? null : new DataBuffer.LoadingListener() {
				@Override
				public void onLoaded(int loadedBytes) {
					listener.onLoaded(packet, loadedBytes + 8);
				}
			};
	...
	protected PtpIpPacket(long packetType) {
	...
	protected abstract static class InitPacket extends PtpIpPacket {
		protected InitPacket(int packetType) {
			super(packetType);
		}
	}
	...
	public static class InitCommandRequest extends InitPacket {
		...
		public String toString() {
			String guidString = "";
			for (int i = 0; i < 16; i++)
				guidString += Integer.toHexString(mGuid[i]) + ":";
			return super.toString() + ":[GUID: " + guidString + ", FriendlyName: " + mFriendlyName +
				", Version: " + mProtVersionMajor + "." + mProtVersionMinor + "]";
		}

		public InitCommandRequest() {
			super(PKT_InitCommandRequest);
		}
		public InitCommandRequest(PtpIpConnection.PtpIpHostId hostId) {
			...
			for (int i = 0; (i < mGuid.length) && (i < hostId.mGuid.length); i++)
				mGuid[i] = hostId.mGuid[i];
		}
	}
	...
	public static class InitCommandAck extends InitPacket {
		...
		@Override
		public void writePayload(DataBuffer out) {
		...
		@Override
		public String toString() {
			return super.toString() + ":[ConnectionNumber: " + mConnectionNumber + "]:[" +
				mResponseData.toString() + "]";
		}

		public InitCommandAck() {
			super(PKT_InitCommandAck);
		}
	...
	public static class InitEventRequest extends InitPacket {
		...
		public InitEventRequest() {
			super(PKT_InitEventRequest);
		}
		public InitEventRequest(long connectionNumber) {
			this();
			mConnectionNumber = connectionNumber;
		}
	}
	...
	public static class InitFail extends InitPacket {
		...
		@Override
		public void writePayload(DataBuffer out) {
			out.writeUInt32(mReason);
		}
		@Override
		protected void readPayload(DataBuffer in) throws PtpIpExceptions.MalformedPacket {
			mReason = in.readUInt32();
		}

		@Override
		public String toString() {
			return super.toString() + ":[Reason: " + String.format("0x%08x", mReason) + "]";
		}

		public InitFail() {
			super(PKT_InitFail);
		}
		public InitFail(long reason) {
			this();
			mReason = reason;
		}
	}
	...
	protected static abstract class TransactionPacket extends PtpIpPacket {
		...
		@Override
		protected void writePayload(DataBuffer out) {
			out.writeUInt32(mTransactionId);
		}
		@Override
		protected void readPayload(DataBuffer in) throws PtpIpExceptions.MalformedPacket {
			mTransactionId = in.readUInt32();
		}
		@Override
		public String toString() {
			return super.toString() + ":[TransactionId: " + mTransactionId + "]";
		}

		protected TransactionPacket(long packetType) {
			super(packetType);
		}
	}
	...
	public static class OperationRequest extends TransactionPacket {
		...
		protected void readPayload(DataBuffer in) throws PtpIpExceptions.MalformedPacket {
			...
			if ((in.available() > 20) || ((in.available() % 4) != 0))
				throw new PtpIpExceptions.MalformedPacket(
					"Malformed OperationsRequest parameters! (" + in.available() + "bytes)");
			mParameters = new long[in.available() / 4];
			for (int i = 0; i < mParameters.length; i++)
				mParameters[i] = in.readUInt32();
		}
		@Override
		public String toString() {
			return super.toString() +
				":[DataPhaseInfo: " + String.format("0x%08x", mDataPhaseInfo) +
				", OperationCode: " + String.format("0x%04x", mOperationCode) +
				", Params: " + arrayToString(mParameters) + "]";
		}

		public OperationRequest() {
			super(PKT_OperationRequest);
		}
		public OperationRequest(boolean dataOut, int operationCode, long transactionId,
								long[] parameters) {
			this();
	...
	public static class OperationResponse extends TransactionPacket {
		...
		protected void readPayload(DataBuffer in) throws PtpIpExceptions.MalformedPacket {
			...
			if ((in.available() > 20) || ((in.available() % 4) != 0))
				throw new PtpIpExceptions.MalformedPacket(
					"Malformed OperationsResponse parameters! (" + in.available() + "bytes)");
			mParameters = new long[in.available() / 4];
			for (int i = 0; i < mParameters.length; i++)
				mParameters[i] = in.readUInt32();
		}
		@Override
		public String toString() {
			return super.toString() + ":[ResponseCode: " + String.format("0x%04x", mResponseCode) +
				", Params: " + arrayToString(mParameters) + "]";
		}

		public OperationResponse() {
			super(PKT_OperationResponse);
		}
	...
	public static class Event extends TransactionPacket {
		...
		protected void readPayload(DataBuffer in) throws PtpIpExceptions.MalformedPacket {
			...
			super.readPayload(in);
			if ((in.available() > 12) || ((in.available() % 4) != 0))
				throw new PtpIpExceptions.MalformedPacket("Malformed Event parameters! (" +
														  in.available() + "bytes)");
			mParameters = new long[in.available() / 4];
			for (int i = 0; i < mParameters.length; i++)
				mParameters[i] = in.readUInt32();
		...
		@Override
		public String toString() {
			return super.toString() + ":[EventCode: " + String.format("0x%04x", mEventCode) +
				", Params: " + arrayToString(mParameters) + "]";
		}
		...
		public Event(int eventCode, long transactionId, long[] parameters) {
			...
			mTransactionId = transactionId;
			for (int i = 0; (i < mParameters.length) && (i < parameters.length); i++)
	...
	public static class StartData extends TransactionPacket {
		...
		@Override
		public String toString() {
			return super.toString() + ":[DataLength: " + mDataLength + "]";
		}
	...
	public static class Data extends TransactionPacket {
		...
		@Override
		public String toString() {
			return super.toString() + ":[PayloadLength: " + mDataPayload.length + "]";
		}
	...
	public static class Cancel extends TransactionPacket {
		...
		public Cancel(long transactionId) {
			this();
			mTransactionId = transactionId;
		}
	}
	...
	public static class InternalPacket extends PtpIpPacket {
		public InternalPacket(long packetType) {
			super(packetType);
		}
	}
	...
	public static class Error extends InternalPacket {
		...
		@Override
		public String toString() {
			return super.toString() + ":[Exception: " + mException + "]";
		}
	...
	public static class LoadStatus extends InternalPacket {
		...
		public LoadStatus(PtpIpPacket packet, int loadedBytes) {
			this();
			mLoadedPacket = packet;
