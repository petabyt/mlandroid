...
public class PtpConnection {
	...
	public List<PtpSession> getSessions() {
		return new ArrayList<>(mPtpSessions);
	}

	public void registerListener(EventCallbacks listener) {
		mListener = listener;
	}
	...
	protected void onSessionClosed(PtpSession session) {
		mPtpSessions.remove(session);
	}
	...
	public void close() throws PtpTransport.TransportError, PtpExceptions.PtpProtocolViolation {
		...
		if (mEventListener != null)
			mEventListener.interrupt();

		// now close all sessions
		try {
			for (PtpSession session : mPtpSessions)
				session.close();
			mPtpSessions.clear();
		}
