...
public abstract class PtpDataType {
	...
	protected abstract void read(PtpTransport.PayloadBuffer in)
		throws PtpTransport.TransportDataError, PtpExceptions.MalformedDataType;
	@Override
	public String toString() {
		return "";
	}

	public void writeToBuffer(PtpTransport.PayloadBuffer out) {
		write(out);
	}
	...
	public static class UInt16 extends PtpDataType implements Serializable {
		...
		@Override
		public String toString() {
			return String.format("0x%04x", mValue);
		}
		public UInt16() {
		}
	...
	public static class UInt32 extends PtpDataType implements Serializable {
		...
		@Override
		public String toString() {
			return String.format("0x%08x", mValue);
		}
		public UInt32() {
		}
	...
	public static class UInt64 extends PtpDataType implements Serializable {
		...
		@Override
		public String toString() {
			return String.format("0x%016x", mValue);
		}
		public UInt64() {
		}
			...
			for (int i = 0; i < (length - 1); i++) {
				...
				if (stringBuffer[i] == 0)
					throw new PtpExceptions.MalformedDataType(
						"PtpString null-terminated before stated length!");
			}
			if (in.readUInt16() != 0)
				throw new PtpExceptions.MalformedDataType(
					"PtpString not null-terminated at stated length!");
	...
	public static class ArrayType<DT extends PtpDataType>
		extends PtpDataType implements Serializable {
		protected DT[] mArrayData;

		@Override
		protected void write(PtpTransport.PayloadBuffer out) {
			out.writeUInt32(mArrayData.length);
			for (DT dt : mArrayData)
				dt.write(out);
		}
		@Override
		protected void read(PtpTransport.PayloadBuffer in)
			...
			for (int i = 0; i < arrayLength; i++) {
				try {
					mArrayData[i] = (DT)dtClass.newInstance();
		...
		public ArrayType(Class dtClass) {
			mArrayData = (DT[])Array.newInstance(dtClass, 0);
		}
	}
	...
	public static class ObjectFormatCode extends Datacode implements Serializable {
		...
		public ObjectType getType() {
			...
			if ((mValue >= 0x3000) && (mValue <= 0x300c))
				return ObjectType.NON_IMAGE;
			if ((mValue & 0x0800) != 0)
		...
		public ObjectFormatCode(int value) {
			mValue = value;
		}
	}
	...
	public static class ObjectHandle extends UInt32 implements Serializable {
		public ObjectHandle(long value) {
			mValue = value;
		}
		public ObjectHandle() {
		}
	...
	public static class AssociationCode extends UInt16 implements Serializable {
		public AssociationCode(int value) {
			mValue = value;
		}
	...
	public static class AssociationDesc extends UInt32 implements Serializable {
		public AssociationDesc(int value) {
			mValue = value;
		}
	...
	public static class Object extends PtpDataType implements Serializable {
		...
		public String toString() {
			String objectString = "";
			for (int i = 0; (i < mObject.length) && (i < 16); i++)
				objectString += String.format("%02x ", mObject[i]);
			if (mObject.length > 16)
				objectString += "+ " + (mObject.length - 16) + " more bytes";
