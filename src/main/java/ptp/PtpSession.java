...
public class PtpSession {
	...
	public long getNumObjects(PtpDataType.StorageID storageId,
		...
		request.mParameters =
			new long[] { storageId.mValue, objectFormat.mValue, associationHandle.mValue };
		PtpOperation.Response response = mSession.executeTransaction(request);
		response.validate();
		if (!response.isSuccess())
	...
	public byte[] getObject(PtpDataType.ObjectHandle objectHandle, final DataLoadListener listener)
		...
		request.mParameters = new long[] { objectHandle.mValue };
		PtpOperation.Response response = mSession.executeTransaction(
			request, listener == null ? null : new PtpTransport.Session.DataLoadListener() {
				@Override
				public void onDataLoaded(PtpOperation.Request request, long loaded, long expected) {
