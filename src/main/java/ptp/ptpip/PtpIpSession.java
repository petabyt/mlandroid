...
public class PtpIpSession implements PtpTransport.Session {
	...
	public PtpIpSession(PtpIpConnection ptpIpConnection,
						BlockingDeque<PtpIpPacket> transactionPacketInQueue) {
	...
	private void testStatus(TransactionStatus currentStatus, TransactionStatus expectedStatus,
							PtpIpPacket packet) throws PtpIpExceptions.ProtocolViolation {
	...
	private void testStatus(TransactionStatus currentStatus, TransactionStatus[] expectedStatus,
		...
		for (TransactionStatus status : expectedStatus)
			if (currentStatus == status)
				return;
		throw new PtpIpExceptions.ProtocolViolation(
			"Wrong PacketType: Received \"" + packet.getClass().getSimpleName() +
			"\" but currently in transaction status \"" + currentStatus.name() + "\"");
	}

	@Override
	public PtpOperation.Response executeTransaction(PtpOperation.Request request)
		throws PtpIpExceptions.MalformedPacket, PtpIpExceptions.ProtocolViolation,
			   PtpTransport.TransportIOError, PtpIpExceptions.OperationFailed {
		return executeTransaction(request, null);
	}
	@Override
	public synchronized PtpOperation.Response executeTransaction(PtpOperation.Request request,
	...
	protected PtpOperation.Response executeNullTransaction(PtpOperation.Request request)
		throws PtpIpExceptions.MalformedPacket, PtpIpExceptions.ProtocolViolation,
			   PtpTransport.TransportIOError, PtpIpExceptions.OperationFailed {
		return executeTransaction(request, 0, null);
	}
	private synchronized PtpOperation.Response
	executeTransaction(PtpOperation.Request request, long transactionId, DataLoadListener listener)
		throws PtpIpExceptions.MalformedPacket, PtpIpExceptions.ProtocolViolation,
			   PtpTransport.TransportIOError, PtpIpExceptions.OperationFailed {
		...
		if (!mIsOpened && (transactionId != 0))
			throw new PtpIpExceptions.ProtocolViolation(
				"Cannot execute transactions without opening session first!");
		...
		PtpIpPacket.OperationRequest requestPacket = new PtpIpPacket.OperationRequest(
			request.hasData(), request.getOperationCode(), transactionId, request.getParameters());
		try {
			mPtpIpConnection.sendCommandChannelPacket(requestPacket);
			if (request.hasData()) {
				DataBuffer dataBuffer = new DataBuffer();
				request.getData().writeToBuffer(dataBuffer);
				mPtpIpConnection.sendCommandChannelPacket(
					new PtpIpPacket.StartData(transactionId, dataBuffer.size()));
				mPtpIpConnection.sendCommandChannelPacket(
					new PtpIpPacket.Data(transactionId, dataBuffer));
				...
				mPtpIpConnection.sendCommandChannelPacket(
					new PtpIpPacket.EndData(transactionId, dataBufferEmpty));
			}
		} catch (IOException e) {
			throw new PtpIpExceptions.IOError(e);
		}
		...
		while (status != TransactionStatus.RESPONSE_RECEIVED) {
			...
			while (packet == null) {
				try {
					packet = mTransactionPacketInQueue.take();
			...
			if (packet instanceof PtpIpPacket.LoadStatus) {
				...
				if ((subPacket instanceof PtpIpPacket.Data) ||
					(subPacket instanceof PtpIpPacket.EndData)) {
					testStatus(status,
							   new TransactionStatus[] { TransactionStatus.DATA_STARTED,
														 TransactionStatus.DATA_ENDED },
							   packet);
					if (listener != null)
						listener.onDataLoaded(request,
											  dataIn.size() +
												  ((PtpIpPacket.LoadStatus)packet).mLoadedBytes - 8,
											  dataIn.size() + dataRemaining);
				}
			}
			...
			// if Event (must be cancel) --> abort if right transaction ID
				...
				if (((PtpIpPacket.Event)packet).mEventCode != PtpEvent.EVENTCODE_CancelTransaction)
					throw new PtpIpExceptions.ProtocolViolation(
						"Non-cancel event packet in transaction queue!");
				if (((PtpIpPacket.Event)packet).mTransactionId >
					transactionId) // this shouldn't happen but let's make sure
					throw new PtpIpExceptions.ProtocolViolation(
						"Received cancel-packet for future transaction!");
				if (((PtpIpPacket.Event)packet).mTransactionId < transactionId)
					continue; // must refer to an old transaction that finished - ignore
			...
			// if TransactionPacket --> check transactionId and process packet-types
				...
				if (((PtpIpPacket.TransactionPacket)packet).mTransactionId != transactionId)
					throw new PtpIpExceptions.ProtocolViolation(
						"Received wrong transaction-Id. Expected " + transactionId + ", received " +
						((PtpIpPacket.TransactionPacket)packet).mTransactionId);
				...
				if (packet instanceof PtpIpPacket.StartData) {
					...
					if (((PtpIpPacket.StartData)packet).mDataLength == 0xFFFFFFFF)
						throw new PtpIpExceptions.ProtocolViolation(
							"Unknown data length (0xFFFFFFFF) currently not supported!");
				...
				// if Data --> check state and receive
					...
					if (dataRemaining < 0)
						throw new PtpIpExceptions.ProtocolViolation(
							"Received Data but longer than announced!");
				...
				// if EndData --> check state and move to response expected
					...
					if (dataRemaining != 0)
						throw new PtpIpExceptions.ProtocolViolation(
							"Received EndData but was expecting " + dataRemaining +
							" more; (EndData payload: " + payload.length);
					dataIn.writeObject(payload);
				...
				// if OperationResponse --> check state and complete transaction
					...
					testStatus(status,
							   new TransactionStatus[] { TransactionStatus.REQUEST_SENT,
														 TransactionStatus.DATA_ENDED },
							   packet);
					PtpIpPacket.OperationResponse ptpIpResponse =
						(PtpIpPacket.OperationResponse)packet;
					response.setResponseCode(ptpIpResponse.mResponseCode);
					response.setParameters(ptpIpResponse.mParameters);
					if (dataIn != null)
						response.setData(dataIn);
	...
	@Override
	public void close()
		throws PtpTransport.TransportDataError, PtpTransport.TransportIOError,
			   PtpTransport.TransportOperationFailed, PtpExceptions.PtpProtocolViolation {
		if (!isOpened())
			return;
		PtpOperation.Response response =
			executeTransaction(PtpOperation.createRequest(PtpOperation.OPSCODE_CloseSession));
		response.validate();
		if (!response.isSuccess())
			throw new PtpIpExceptions.OperationFailed("CloseSession", response.getResponseCode());
		mIsOpened = false;
	}
}
