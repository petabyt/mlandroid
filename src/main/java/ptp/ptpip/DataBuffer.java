...
public class DataBuffer implements PtpTransport.PayloadBuffer {
	...
	public DataBuffer(boolean isLittleEndian) {
		this(null, 0, isLittleEndian);
	}
	public DataBuffer(int size) {
		this(null, size, true);
	}
	public DataBuffer(int size, boolean isLittleEndian) {
		this(null, size, isLittleEndian);
	}
	public DataBuffer(byte[] packetData) {
		this(packetData, 0, true);
	}
	public DataBuffer(byte[] packetData, boolean isLittleEndian) {
		this(packetData, 0, isLittleEndian);
	}
	...
	private void startRead(int length) throws PtpIpExceptions.MalformedPacket {
		if (mStatus != Status.READ) {
			mInData = mOut.toByteArray();
			mIn = new ByteArrayInputStream(mInData);
			mOut = null;
			mStatus = Status.READ;
		}
		if (mIn.available() < length)
			throw new PtpIpExceptions.MalformedPacket("Insufficient data in buffer (expected " +
													  length + " bytes)!");
	}

	interface LoadingListener {
		void onLoaded(int loadedBytes);
	}
	public void fill(InputStream inputStream, int length) throws IOException {
		fill(inputStream, length, null);
	}
	public void fill(InputStream inputStream, int length, LoadingListener listener)
		throws IOException {
	...
	public byte[] getData() {
		return mStatus == Status.READ ? mInData : mOut.toByteArray();
	}
	...
	public long size() {
		return mStatus == Status.WRITE ? mOut.size() : mInData.length;
	}

	@Override
	public DataBuffer writeUInt8(short uint8) {
		startWrite();
		mOut.write(uint8);
		return this;
	}
	...
	@Override
	public DataBuffer writeUInt16(int uint16) {
		startWrite();
		if (mIsLE) {
			mOut.write(uint16);
			mOut.write(uint16 >> 8);
	...
	@Override
	public DataBuffer writeUInt32(long uint32) {
		startWrite();
		if (mIsLE) {
			writeUInt16((int)(uint32 & 0x0000FFFF));
			writeUInt16((int)(uint32 >> 16));
		} else {
			writeUInt16((int)(uint32 >> 16));
			writeUInt16((int)(uint32 & 0x0000FFFF));
		}
	...
	@Override
	public DataBuffer writeUInt64(long uint64) {
		startWrite();
		if (mIsLE) {
			writeUInt32(uint64 & 0xFFFFFFFF);
			writeUInt32(uint64 >> 32);
		} else {
			writeUInt32(uint64 >> 32);
			writeUInt32(uint64 & 0xFFFFFFFF);
		}
	...
	public DataBuffer writeUtf16String(String string) {
		...
		for (int i = 0; i < string.length(); i++)
			writeUInt16(string.charAt(i));
		writeUInt16(0);
		return this;
	}

	@Override
	public DataBuffer writeObject(byte[] byteArray) {
		startWrite();
		if (byteArray == null)
			return this;
	...
	public String readUtf16String() throws PtpIpExceptions.MalformedPacket {
		...
		throw new PtpIpExceptions.MalformedPacket(
			"PtpIp-string longer than internal limit of 100 chars!");
	}
